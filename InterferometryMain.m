...
% Check if numbers
if min(cellfun(@(x) isnumeric(x), {Lambda, PlotSingleSlice, NumberSlices, SectorStart, SectorEnd})) == 0
    Logging(1, 'The settings "lambda, plot_line, slices, quadrant_start, quadrant_end, MaxDeviation" should be numerics.')
end

% Check booleans
if min(cellfun(@(x) islogical(x), {AnalyzeSector, ShowHeightProfileProgress, FilterBy_AmountExtrema, Plot_Average})) == 0
Logging(1, 'The settings "analyze_quadrant, ShowInvidualProgress, FilterBy_AmountExtrema, PlotAverage".')
end
...
if SectorEnd - SectorStart < 0
    Loggin(1, 'Negative quadrants are not (yet) supported. quadrant_end > quadrant_start.')
end
